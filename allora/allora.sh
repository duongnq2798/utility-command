...
mkdir worker-data1 worker-data2 worker-data3 worker-data7 worker-data10 worker-data11 head-data
sudo chmod -R 777 worker-data1 worker-data2 worker-data3 worker-data7 worker-data10 worker-data11 head-data
...
sudo docker run -it --entrypoint=bash -v $(pwd)/worker-data10:/data alloranetwork/allora-inference-base:latest -c "mkdir -p /data/keys && (cd /data/keys && allora-keys)"
sudo docker run -it --entrypoint=bash -v $(pwd)/worker-data11:/data alloranetwork/allora-inference-base:latest -c "mkdir -p /data/keys && (cd /data/keys && allora-keys)"
    ...
    entrypoint:
      ...
      - |
        ...
        allora-node --role=worker --peer-db=/data/peerdb --function-db=/data/function-db \
          ...
          --allora-chain-key-name=worker-10
    volumes:
      - ./worker-data10:/data
    working_dir: /data
    depends_on:
    ...
    entrypoint:
      ...
      - |
        ...
        allora-node --role=worker --peer-db=/data/peerdb --function-db=/data/function-db \
          ...
          --allora-chain-key-name=worker-11
    volumes:
      - ./worker-data11:/data
    working_dir: /data
    depends_on:
